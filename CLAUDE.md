...
**llama.cpp on the box.** Use **`D:\llama-b10078\`** (2026-07-21): its ggml-vulkan memory-type-selection fix places the whole model in the dedicated carve instead of spilling ~16 GB to host-visible/GTT (Windows RAM free 7.2 → 22.3 GB with Coder-Next serving; box desktop workloads now coexist). Launch with env `GGML_VK_DISABLE_HOST_VISIBLE_VIDMEM=1` + `--no-mmap` (validated V8 config, see `D:\claude-coordination\QWEN_SERVER_NEGOTIATION.md`). Older builds: b9596/b9611 (qwen3_next-capable but spill ~16 GB to GTT), b9090 (crashes Vulkan compute on qwen3_next; oldest-arch fallback only). Drop `--kv-unified` (kept off since the b9090 cancel-task stall bug); `--cache-reuse` stays dropped (bead 081 agentic stall).

**Box GPU memory.** Never co-load two large models — coder-box (48.4 GB in-carve) + gpt-oss-120B (61.4 GB) exceeds the 96 GB carve (beads-081 crash class). On b10078 the old "≥ ~58 GB single-model OOMs at submit" limit is GONE: gpt-oss-120B mxfp4 (63 GB, 3 shards) loads clean, fully in-carve, 51.6 tps gen — it is the validated generalist-on-demand model (swap with coder ~1 min). Historical (pre-b10078) load-order quirk: coder before vision, ≥58 GB OOMs.
