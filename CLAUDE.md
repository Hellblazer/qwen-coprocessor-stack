...
**qwen_dispatch is opt-in per session (bead n8c).** The tool returns `dispatch_not_enabled` unless the session's supervisor was started with `QWEN_DISPATCH_ENABLE=1`. Each Claude Code session spawns its own supervisor from the plugin manifest and that process inherits the launching shell's env (verified 2026-09-14: the live plugin supervisor carries the shell's `PWD`/`TERM_PROGRAM`), so `QWEN_DISPATCH_ENABLE=1 QWEN_AGENT_PROVIDERS='[{"id":"box","agentKind":"qwen-local","backend":"coder-box"}]' claude` scopes dispatch and its provider to that one session. **Pin `backend` on the provider (bead 11r):** without it each dispatch spawn takes the shared weighted round-robin, and with coder-box and glm-openrouter both remote/heavy/weight 1 every other `qwen-local` dispatch runs on paid OpenRouter (WARN `dispatch_backend_unpinned`). A pin naming a backend not in the pool fails with `backend_unavailable`. Dispatch reports `cost: 0` for any backend, pinned or not. For two backends in one session, declare two providers and choose with `provider_id`. **Never add `agent_providers` to `~/.qwen-coprocessor-stack/config.json`**: every session's supervisor reads that file. The gate is checked before provider lookup, so a config-file provider cannot enable dispatch anyway. The plugin runs the published npm package, so the gate is live only from the release that carries it. **The extension loadout is per-session the same way:** `QWEN_DEFAULT_EXTENSIONS='qwen-toolkit'` in that env gives the session's dispatch spawns an exact extension set (dispatch passes no extension opts, so the session default resolves), and since an installed extension is enabled by default, install it default-off with `{"qwen-toolkit": {"overrides": ["!/**"]}}` in `~/.qwen/extensions/extension-enablement.json` — an explicitly named extension still loads despite the disable (measured 2026-09-15). Recipe in `docs/USER_GUIDE.md`.

**Dogfood protocol (bead 3su.18).** Label beads whose work a Qwen dispatch did with `qwen-dogfood`; append the one-line `bd note` outcome record (family, outcome, toolkit version or none, backend, wall, turns, and mandatory `wrong=` text on accepted-with-edits/rejected); a QWEN.md or agents change bumps the toolkit manifest version and triggers a battery re-run; full format in the `docs/USER_GUIDE.md` dogfood recipe.
