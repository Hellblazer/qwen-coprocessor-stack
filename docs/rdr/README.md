...
| 013 | closed (implemented)  | design       | [Per-spawn MCP server forwarding — let qwen_spawn/qwen_oneshot provision the inner qwen-code agent with per-task MCP tools](RDR-013-per-spawn-mcp-forwarding.md) |
...
| 014 | closed (implemented)  | design       | [agent-lsp opt-in code-intelligence provider — one-flag opts.codeIntel that injects agent-lsp, its symbol-graph guidance, and a default tool budget into a spawn](RDR-014-agent-lsp-codeintel-optin.md) |
| 015 | accepted              | design       | [codeIntel agent-lsp daemon lifecycle — document TS/Go daemon-broker warm reuse and the in-process jdtls model; defer any spawn-concurrency cap](RDR-015-codeintel-agent-lsp-daemon-lifecycle.md) |
| 016 | Closed | design       | [Model and runtime provenance — pinned-source fetch, hash-verified lineage, committed manifest, hooks/skills that make the unverified path the hard path](RDR-016-model-binary-provenance.md) |
