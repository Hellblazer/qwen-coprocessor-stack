...
const CODEINTEL_GUIDANCE = [
  ...
  "- Call `start_lsp` FIRST — every other tool errors with \"LSP client not",
  "  initialized\" until you do. Pass `root_dir` = the directory that holds the",
  "  build manifest (`package.json`/`tsconfig.json`, `go.mod`, `Cargo.toml`,",
  "  `pyproject.toml`, …). This is normally your working directory; in a monorepo",
  "  whose root has NO manifest it is a SUBDIRECTORY under it, not the repo root.",
  "  If `start_lsp` itself returns \"No Project\", your `root_dir` has no manifest —",
  "  fix it and re-call `start_lsp`. Pass `ready_timeout_seconds: 30` (raise to",
  "  60–120 for large repos) so indexing finishes before you query, and `scope`",
  "  to a subdir on a large monorepo to keep it fast.",
  "- `find_symbol` is a WORKSPACE-symbol search. If `start_lsp` SUCCEEDED but",
  "  `find_symbol` returns \"No Project\" or empty (typescript-language-server does",
  "  this until a file in the project is open), open one known file first with",
  "  `list_symbols` on its path, then retry `find_symbol` — do NOT fall back to",
  "  text grep. (`list_symbols` is the reliable opener; `go_to_definition` /",
  "  `get_symbol_source` need a position you do not have yet.)",
